...
###########################################################
    ...
    def cmdToChannel(self, cmd):
        """
        Check if the command requires to switch on/off a converter.

            Args: 
            - Command

            Return:
            - Boolean (true if converters are involved)
            - GPIO channel
        """
        if (any(status in cmd for status in ('ON','OFF'))) and (not any(dev in cmd for dev in ('OBC1','OBC2'))): 
            return True, self.cmdDict[cmd]['CHANNEL']
